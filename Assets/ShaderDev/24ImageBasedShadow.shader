...
Shader"ShaderDev/24ImageBasedShadow"
{
	Properties
	{
		...
		_RefractionFactor("Reflection %", Range(0, 1)) = 1
	...
	{	
		...
		{
			NAME "ShadowCaster"
			Tags{"Quene" = "Opaque" "LightMode" = "ShadowCaster"}
			ZWrite On
			Cull off
		...
		Pass
		{
			...
			vertexOutput vert(vertexInput i)
			{
				...
				float3 worldSpaceViewDir = normalize(_WorldSpaceCameraPos - o.worldPos);
					...
					#if _IBLMODE_REFR						
						float3 worldRefract = refract(-worldSpaceViewDir, o.normalWorld.xyz, 1/_RefractiveIndex);	// eta = index1/index2
						o.surfaceColor.rgb *= IBLRefl(_CubeMap, _Detail, worldRefract, _ReflectionExposure, _RefractionFactor);
					#endif

					#if _IBLMODE_FRES
						float3 worldReflection = reflect(-worldSpaceViewDir, o.normalWorld.xyz);
						float3 reflectionColor = IBLRefl(_CubeMap, _Detail, worldReflection, _ReflectionExposure, _ReflectionFactor);
					...
					#if _IBLMODE_REFR	
						float3 worldRefract = refract(-worldSpaceViewDir, o.normalWorld.xyz, 1/_RefractiveIndex);
						o.surfaceColor.rgb += IBLRefl(_CubeMap, _Detail, worldRefract, _ReflectionExposure, _RefractionFactor);
					#endif
				#endif
				return o;
			...
			half4 frag(vertexOutput o): COLOR
			{
				float4 finalColor = float4(0, 0, 0, _Color.a);
					...
					#if _SHADOWMODE_ON
						finalColor.rgb *= tex2D(_ShadowMapTexture, o.shadowCoord).a;
					#endif
					...
					#if _IBLMODE_REFR
						float3 worldRefract = refract(-worldSpaceViewDir, normalAtWorld, 1/_RefractiveIndex);
						finalColor.rgb *= IBLRefl(_CubeMap, _Detail, worldRefract, _ReflectionExposure, _RefractionFactor);
					#endif
					...
					finalColor = o.surfaceColor;
					...
					#if _IBLMODE_REFL	
						float3 worldSpaceViewDir = normalize(_WorldSpaceCameraPos - o.worldPos);
						float3 worldReflection = refract(-worldSpaceViewDir, normalAtWorld, 1/_RefractiveIndex);
						finalColor.rgb += IBLRefl(_CubeMap, _Detail, worldReflection, _ReflectionExposure, _RefractionFactor);
					#endif
						...
						finalColor.rgb = lerp(texColor * _Color.rgb, finalColor.rgb + reflectionColor, fresnel);
					#endif
					//return half4(normalAtWorld.xyz, 1);
				#endif
