...
Shader"ShaderDev/09BlinkingBokeh"
{
	// 属性
	Properties
	{
		...
		_Feather("Circle Fade Feather", Range(0.001, 1.1)) = 0.05
		_Speed("Speed", Range(0.5, 3)) = 1
	}
	...
	{	
		...
		Pass
		{
			...
			float drawCircleBlink(float2 uv, float2 center, float radius, float feather)
			{
				float circle = pow((uv.y - center.y), 2) + pow((uv.x - center.x), 2);
				float radiusSqr = pow(radius,2);
				float BlinkValue =  abs(sin(_Time.y*_Speed));
				// if (circle < radiusSqr)
				// 	return smoothstep(radiusSqr, radiusSqr - feather, circle);
				// return 0;
				return BlinkValue * smoothstep(radiusSqr, radiusSqr - feather, circle);
			}
			half4 frag(vertexOutput o): COLOR
			{
				...
				col.a = drawCircleBlink(o.texCoord.xy, _Center, _Radius,_Feather);
