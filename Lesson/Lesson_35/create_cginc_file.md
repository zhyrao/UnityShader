### Create CG cginc File
在本章中我们将学习如何编写及使用cginc文件。我们还是以上次已经做好的UseNormalMap为例子。目标就是把一些在normal map中的函数提取出来放入cginc文件中，然后在使用这个文件来达到同样的效果。

1. 首先先创建一个名称为CVGLighting.cginc的文件。
2. 然后将normalmap shader中通用的函数部分放置到cginc文件中。
...
Shader"ShaderDev/16NormalMapping_UsingCGFile"
{
    ...
    {   
        ...
        Pass
        {
            ...
            CGPROGRAM
            #include "CVGLighting.cginc"    // 使用cginc文件
            #pragma vertex vert 
